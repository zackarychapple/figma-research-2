...
export class ComponentMatcher {
    ...
    apiKey;
    thresholds;
    constructor(dbPath, thresholds = { exactMatch: 0.85, similarMatch: 0.75 }) {
     ...
     */
    extractComponentText(component) {
        const parts = [];
     ...
     */
    async indexComponent(component) {
        // Insert component
     ...
     */
    async findMatches(queryComponent, options = {}) {
     ...
     */
    async validateAccuracy(testPairs) {
        ...
        for (const testPair of testPairs) {
            ...
            if (predictedType === testPair.expectedType) {
                if (predictedType === 'exact')
                    exactMatchCorrect++;
                else if (predictedType === 'similar')
                    similarMatchCorrect++;
                else if (predictedType === 'none')
                    noMatchCorrect++;
            }
            else {
                // Analyze error type
     ...
     */
    async indexComponentsBatch(components) {
 ...
 */
export function createTestComponent(name, type = 'COMPONENT', metadata = {}) {
    return {
 ...
 */
export function createButtonComponent(variant, size = 'medium') {
    const sizeMap = {
    ...
    return createTestComponent(`Button / ${variant}`, 'COMPONENT', {
 ...
 */
export function createCardComponent(variant) {
    return createTestComponent(`Card / ${variant}`, 'COMPONENT', {
        width: 300,
        height: 400,
        childCount: 4,
