...
export class ColorExtractor {
     ...
     */
    static extractColor(figmaColor, opacity = 1, type = 'fill') {
...
export class TypographyExtractor {
     ...
     */
    static mapFontWeight(fontStyle) {
        const weightMap = {
            'Thin': 100,
            'Extra Light': 200,
            'ExtraLight': 200,
...
export class EffectsExtractor {
    ...
    static extractEffects(node) {
        ...
        return node.effects
            ...
            if (effect.color) {
                extracted.color = ColorExtractor.extractColor(effect.color, effect.color.a, 'shadow');
            }
            if (effect.offset) {
                extracted.offset = {
                    x: effect.offset.x || 0,
     ...
     */
    static effectsToBoxShadow(effects) {
...
export class SpacingExtractor {
     ...
     */
    static isUniformPadding(spacing) {
     ...
     */
    static isSymmetricPadding(spacing) {
        const { top, right, bottom, left } = spacing.padding;
...
export class ComponentClassifier {
    ...
    static classifySelect(node) {
        ...
        const hasText = node.children?.some(c => c.type === 'TEXT');
        const hasIcon = node.children?.some(c => c.name.toLowerCase().includes('icon') ||
    ...
    static classifyCard(node) {
        ...
        const hasShadow = node.effects && node.effects.some(e => e.type === 'DROP_SHADOW' && e.visible !== false);
...
export class TailwindMapper {
     ...
     */
    static mapColorToTailwind(color, prefix) {
     ...
     */
    static mapSpacing(padding, prefix) {
        ...
        if (top === bottom && left === right) {
            const classes = [];
            if (top > 0)
                classes.push(this.spacingValueToClass(top, `${prefix}y`));
            if (left > 0)
                classes.push(this.spacingValueToClass(left, `${prefix}x`));
            return classes;
        }
        // Individual sides
        const classes = [];
        if (top > 0)
            classes.push(this.spacingValueToClass(top, `${prefix}t`));
        if (right > 0)
            classes.push(this.spacingValueToClass(right, `${prefix}r`));
        if (bottom > 0)
            classes.push(this.spacingValueToClass(bottom, `${prefix}b`));
        if (left > 0)
            classes.push(this.spacingValueToClass(left, `${prefix}l`));
        return classes;
    }
    /**
     * Convert spacing value to Tailwind class
     */
    static spacingValueToClass(value, prefix) {
    ...
    static mapTypography(typo) {
        ...
        // Text align
        if (typo.textAlign === 'CENTER')
            classes.push('text-center');
        else if (typo.textAlign === 'RIGHT')
            classes.push('text-right');
        else if (typo.textAlign === 'JUSTIFIED')
     ...
     */
    static mapShadow(effects) {
        const shadows = effects.filter(e => e.type === 'DROP_SHADOW' && e.visible);
        if (shadows.length === 0)
            return null;
    ...
    static mapLayout(layout) {
        ...
        if (layout.mode === 'HORIZONTAL' || layout.mode === 'VERTICAL') {
            ...
            else if (layout.justify === 'SPACE_BETWEEN')
                classes.push('justify-between');
            else if (layout.justify === 'SPACE_AROUND')
...
export class EnhancedFigmaParser {
     ...
     */
    static extractStyles(node) {
        const colors = {
            background: ColorExtractor.extractFills(node),
            text: node.type === 'TEXT' ? [ColorExtractor.extractTextColor(node)].filter(Boolean) : [],
     ...
     */
    static generateCssProperties(styles) {
        const css = {};
        // Background
