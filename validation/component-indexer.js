...
class OpenRouterClient {
     ...
     */
    async generateEmbedding(text, model = 'openai/text-embedding-3-small') {
...
export class ComponentIndexer {
    ...
    openRouter;
    stats;
    constructor(dbPath, openRouterApiKey) {
     ...
     */
    async indexComponents(routeDataPath, options = {}) {
        const { limit = Infinity, skip = 0, componentTypes = ['SYMBOL', 'COMPONENT', 'COMPONENT_SET', 'INSTANCE', 'FRAME'], generateEmbeddings = true, verbose = false } = options;
        this.stats = {
            totalFiles: 0,
            indexed: 0,
            skipped: 0,
     ...
     */
    async indexComponentFile(filePath, componentTypes, generateEmbeddings, verbose) {
     ...
     */
    findComponentFiles(routeDataPath) {
        const files = [];
        const scanDir = (dir) => {
