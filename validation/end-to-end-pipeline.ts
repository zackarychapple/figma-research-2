...
import { EnhancedFigmaParser, EnhancedComponent, FigmaNode } from './enhanced-figma-parser.js';
import { ComponentIndexer } from './component-indexer.js';
import { ComponentMatcher, ComponentMatchResult } from './component-matcher.js';
...
class CodeGenerator {
  ...
  private createPrompt(component: EnhancedComponent, matchResult?: ComponentMatchResult): string {
  ...
  private createNewComponentPrompt(component: EnhancedComponent): string {
...
Return ONLY the TypeScript/React component code. No explanations.`;
  ...
  private createExactMatchPrompt(component: EnhancedComponent, match: Component): string {
...
Return ONLY the TypeScript/React code. No explanations.`;
  ...
  private createSimilarMatchPrompt(component: EnhancedComponent, match: Component): string {
...
export class FigmaToCodePipeline {
  ...
  private cachedParser: CachedFigmaParser;
  ...
  async processComponent(
    ...
    try {
      ...
      const extractedComponent = EnhancedFigmaParser.parseNode(component as FigmaNode);
      ...
      if (options.validate && options.referenceImagePath && options.implementationImagePath) {
        ...
        const validationResult = await compareImages(
          options.referenceImagePath,
          options.implementationImagePath,
          {
   ...
   */
  private mapClassificationToType(classificationType: string): Component['component_type'] {
    const mapping: Record<string, Component['component_type']> = {
      'Button': 'COMPONENT',
      'Input': 'COMPONENT',
      'Card': 'COMPONENT',
      'Badge': 'COMPONENT',
